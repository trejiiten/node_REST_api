  ...
  module.exports = {
      ...
      newUser: async (req,res,next) =>{
        ...
        try {
            ...
            const [user, created] = await User.findOrCreate({where:{username:request.username}, defaults:{
            ...
            if(created){
                 ...
                 res.status(403).json({message: `The user ${request.username} already exists`});
            }
        } catch (error) {
            next(error);
            res.status(500).send();
